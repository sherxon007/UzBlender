...
import { OrbitControls, Stage, Float, Grid as DreiGrid } from '@react-three/drei';
import { Download, ShoppingCart, Shield, Cpu, Box, Layers, ArrowLeft, MessageSquare, Star, Share2, ThumbsUp, Heart, ChevronRight, Eye, Grid, Tag, CheckCircle2, BadgePercent, X, Flag, Send } from 'lucide-react';
import { useStore } from '../../store';
import { Asset } from '../../types';
import { Tooltip } from '../Tooltip';
import { translations } from '../../translations';

// Mock 3D Model Component with interaction
  ...
  return (
    <Float speed={2} rotationIntensity={0.5} floatIntensity={0.5}>
        ...
        <meshStandardMaterial 
            color={texture ? "#2563eb" : "#475569"} 
            wireframe={wireframe}
            emissive={texture ? "#1e40af" : "#000000"}
        ...
        <mesh scale={0.95}>
            <icosahedronGeometry args={[1, 1]} />
            <meshStandardMaterial color="#00f0ff" opacity={0.1} transparent />
        </mesh>
        </mesh>
    </Float>
  );
...
// 3D Viewer Scene with Inspector Controls
const Viewer3D = () => {
        ...
        <div className="w-full h-full bg-[#020408] relative rounded-xl overflow-hidden border border-white/10 group cursor-move shadow-inner">
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-[#0a1220]/90 backdrop-blur-md p-1.5 rounded-lg border border-white/10 flex gap-2 transition-opacity opacity-0 group-hover:opacity-100 z-20 shadow-[0_0_20px_rgba(0,0,0,0.5)]">
                ...
                <Tooltip content={t.fullscreen}>
                    <button className="p-2 hover:bg-white/10 rounded text-gray-400 border border-transparent"><Eye size={16} /></button>
                </Tooltip>
            </div>
...
export const ProductDetails = () => {
    ...
    // Retrieve asset from store instead of mock
                                    ...
                                    <div className="space-y-6 animate-fade-in">
                                        ...
                                        {isOwned && (
                                            <form onSubmit={handleReviewSubmit} className="bg-[#0a1220] p-6 rounded-xl border border-white/10 mb-6">
                                                ...
                                                <div className="flex gap-2 mb-4">
                                                    {[1,2,3,4,5].map(star => (
                                                        <button 
                                                            ...
                                                            className={`transition-colors ${star <= reviewRating ? 'text-yellow-400' : 'text-gray-700 hover:text-yellow-400/50'}`}
                                                        >
                                                ...
                                                <textarea 
                                                    className="w-full bg-[#050b14] border border-white/10 rounded-lg p-3 text-white text-sm focus:outline-none focus:border-cyber-cyan"
                                                    rows={3}
                                                    placeholder="Asset haqida fikringizni yozing..."
                                                    value={reviewText}
                                                    onChange={(e) => setReviewText(e.target.value)}
                                                />
                                                <div className="flex justify-end mt-3">
                                                    <button type="submit" className="bg-cyber-cyan text-black px-4 py-2 rounded-lg font-bold text-sm hover:bg-white transition-colors">
                                                        Yuborish
                                                    </button>
                                                </div>
                                            </form>
                                        ...
                                        <div className="space-y-4">
                                            {(!asset.reviews || asset.reviews.length === 0) && (
                                                <p className="text-gray-500 italic">Hozircha sharhlar yo'q.</p>
                                            )}
                                ...
                                {activeTab === 'qa' && (
                                    <div className="space-y-4 animate-fade-in">
                                        <div className="bg-blue-900/10 border border-blue-500/20 p-4 rounded-xl flex items-start gap-3">
                                            <div className="bg-blue-500/20 p-2 rounded-full text-blue-400 mt-1">
                                            ...
                                            <div>
                                                ...
                                                <button onClick={handleAskQuestion} className="text-xs bg-blue-600 hover:bg-blue-500 text-white px-3 py-1.5 rounded font-bold transition-colors">
                                                    Muallifga Yozish
                                                </button>
                                            </div>
                                        </div>
                    ...
                    <div className="lg:col-span-1">
                        <div className="sticky top-24 space-y-6">
                            ...
                            <div className="bg-[#0a1220] rounded-2xl p-6 border border-white/10 shadow-2xl relative overflow-hidden">
                                {asset.discountPrice && (
                                    <div className="absolute top-0 right-0 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-bl-lg">
                                        CHEGIRMA {Math.round(((asset.price - asset.discountPrice) / asset.price) * 100)}%
                                    </div>
                                )}
                                ...
                                <div className="space-y-3 mb-6">
                                    <div 
                                        className={`p-3 rounded-xl border-2 cursor-pointer transition-all ${licenseType === 'standard' ? 'border-cyber-cyan bg-cyber-cyan/10' : 'border-white/5 hover:border-white/20 bg-[#050b14]'}`}
                                        onClick={() => setLicenseType('standard')}
                                    >
                                        <div className="flex justify-between items-center mb-1">
                                            <span className="font-bold text-sm text-white">Standart Litsenziya</span>
                                            {licenseType === 'standard' && <div className="w-4 h-4 rounded-full bg-cyber-cyan shadow-[0_0_10px_#00f0ff]"></div>}
                                        </div>
                                        <p className="text-xs text-gray-400">Shaxsiy foydalanish va 1 tijoriy loyiha uchun.</p>
                                    </div>
                                    <div 
                                        className={`p-3 rounded-xl border-2 cursor-pointer transition-all ${licenseType === 'extended' ? 'border-cyber-cyan bg-cyber-cyan/10' : 'border-white/5 hover:border-white/20 bg-[#050b14]'}`}
                                        onClick={() => setLicenseType('extended')}
                                    >
                                        <div className="flex justify-between items-center mb-1">
                                            <span className="font-bold text-sm text-white">Kengaytirilgan Litsenziya</span>
                                            {licenseType === 'extended' && <div className="w-4 h-4 rounded-full bg-cyber-cyan shadow-[0_0_10px_#00f0ff]"></div>}
                                        </div>
                                ...
                                <div className="space-y-3">
                                    {isOwned ? (
                                        <button className="w-full bg-green-500 hover:bg-green-400 text-black font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-[0_0_20px_rgba(34,197,94,0.4)]">
                                            <Download size={20} /> Qayta Yuklash
                                        </button>
                                    ) : (
                                        <>
                                            <button 
                                                ...
                                                disabled={isInCart}
                                                className={`w-full font-bold py-4 rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg ${isInCart ? 'bg-white/10 text-gray-400 cursor-not-allowed' : 'bg-cyber-cyan hover:bg-white text-black shadow-[0_0_20px_rgba(0,240,255,0.4)]'}`}
                                            >
                                                {isInCart ? (
                                                    <><CheckCircle2 size={20} /> Savatda</>
                                                ) : (
                                            ...
                                            <button 
                                                onClick={() => {
                                                    if(!user) {
                                                        addToast('Taklif berish uchun tizimga kiring', 'info');
                                                        openAuth('login');
                                                    } else {
                                                        setShowOfferModal(true);
                                                    }
                                                }}
                                                className="w-full bg-transparent text-white border border-white/20 font-bold py-3 rounded-xl hover:border-white/50 hover:bg-white/5 transition-all flex items-center justify-center gap-2"
                                            >
                                                <BadgePercent size={18} /> Narx Taklif Qilish
                                            </button>
                                        </>
                                    )}
                                </div>
                                ...
                                <div className="mt-6 flex items-center justify-between text-gray-500">
                                    <button onClick={() => toggleWishlist(asset.id)} className={`flex items-center gap-2 text-xs font-bold transition-colors ${isLiked ? 'text-pink-500' : 'hover:text-pink-500'}`}>
                                        <Heart size={16} fill={isLiked ? "currentColor" : "none"} /> {isLiked ? 'Saqlandi' : 'Saqlash'}
                                    </button>
                                    <button onClick={() => addToast('Havola nusxalandi!', 'info')} className="flex items-center gap-2 text-xs font-bold hover:text-white transition-colors">
                                        <Share2 size={16} /> Ulashish
                                    </button>
                                    <button onClick={() => addToast('Shikoyat yuborildi.', 'info')} className="flex items-center gap-2 text-xs font-bold hover:text-red-500 transition-colors">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="bg-[#0a1220] p-3 rounded-lg border border-white/10 flex items-center gap-2 text-xs font-bold text-gray-400">
            ...
            {showOfferModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="bg-[#0a1220] border border-white/10 rounded-2xl p-8 max-w-md w-full relative z-10 shadow-2xl animate-scale-in">
                        <button onClick={() => setShowOfferModal(false)} className="absolute top-4 right-4 text-gray-400 hover:text-white">
                            <X size={20} />
                        </button>
                            ...
                            <div className="mb-4">
                                ...
                                <div className="relative">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-[#050b14] border border-white/10 rounded-xl p-3 pl-8 font-bold text-white focus:outline-none focus:border-cyber-cyan"
                                        placeholder={String(asset.price)}
                                    />
                                </div>
                            </div>
                            <button type="submit" className="w-full bg-cyber-cyan text-black font-bold py-3 rounded-xl hover:bg-white transition-colors">
                                {t.submitOffer}
                            </button>
                        </form>
                    </div>
